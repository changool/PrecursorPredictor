...
# This version automatically detect isolation window for DIA
import base64
import struct
import re
import numpy as np
...
for filename in os.listdir(dirpath):
    if filename.endswith(".raw"):
        ...
        left_isolation_window_offset = 0.2 # distance for precursor prediction to the left side from the center of the isolation window. Unit is dalton.
        right_isolation_window_offset = 0 # distance for precursor prediction to the right side from the center of the isolation window. Unit is dalton.
        ms2_generation_iteration_count = 20 # how many times it will iterate for MS2 generation for a certain isolation window
        isoppm = 10 # this is ppm for isotope cluster extraction
        removal_isoppm = 7 # this is ppm for peak removal for the next iteration
        initial_max_peak_intensity_threshold = 10000 # intensity threshold for precursor peak for precursor prediction
        iteration_intensity_ratio_threshold = 0.01 # this parameter determine when to stop finding more precursors than one
        ...
        for num, xx in enumerate(inputfile1): # Input file
            ...
            if '</scan>' in previous1 and '</scan>' in xx:
                ...
                ms2mz1_previous = 0.0
                #print ms1ms2list
                for n1,x1 in enumerate(ms1ms2list):
                    ...
                    if 'filterLine=' in x1 and msLevel1 == 2:
                        ms2mz1_ls = re.findall('ms2 (.*?)@hcd',x1)
                        ms2mz1 = float(ms2mz1_ls[0])
                        IsolationWindowDistance = ms2mz1 - ms2mz1_previous
                        #print ms2mz1
                        if n1 > 0:
                            IsolationWindowList.append(IsolationWindowDistance)
                        ms2mz1_previous = ms2mz1
                #print IsolationWindowList
                IsolationWindow = np.mean(IsolationWindowList)
                        ...
                        while ms2_generation == 'ON' and iteration_count < ms2_generation_iteration_count: # control this parameter
                            ...
                            for y in scan_unit:
                                if '<scan num=' in y:
                                    ...
                                    if iteration_count == 0:
                                        for y1,y2 in zip(peak_list,peak_int):
                                            ...
                                            if y1 > ms2mz + right_isolation_window + 3:
                                                break
                                            if y1 > ms2mz - left_isolation_window and y1 < ms2mz + right_isolation_window + 3:
                                    ...
                                    M1_c2_int = 2*(10**-8)*(ms2_c2**2) + 0.0004*ms2_c2 + 0.2413
                                    M2_c2_int = 2*(10**-7)*(ms2_c2**2) - 0.0002*ms2_c2 + 0.233
                                    M3_c2_int = 2*(10**-7)*(ms2_c2**2) - 0.0003*ms2_c2 + 0.1226
                                    
                                    M1_c3_int = 2*(10**-8)*(ms2_c3**2) + 0.0004*ms2_c3 + 0.2413
                                    M2_c3_int = 2*(10**-7)*(ms2_c3**2) - 0.0002*ms2_c3 + 0.233
                                    M3_c3_int = 2*(10**-7)*(ms2_c3**2) - 0.0003*ms2_c3 + 0.1226                        
                                    
                                    M1_c4_int = 2*(10**-8)*(ms2_c4**2) + 0.0004*ms2_c4 + 0.2413
                                    M2_c4_int = 2*(10**-7)*(ms2_c4**2) - 0.0002*ms2_c4 + 0.233
                                    M3_c4_int = 2*(10**-7)*(ms2_c4**2) - 0.0003*ms2_c4 + 0.1226                        
                                    
                                    #print M1_c2_int,M2_c2_int,M3_c2_int,M1_c3_int,M2_c3_int,M3_c3_int,M1_c4_int,M2_c4_int,M3_c4_int
                
                                    if len(mz_m2_charge2_int)==0:
                                        max_m2_charge2_int = 0.00001
                                    else:
                                        max_m2_charge2_int = max(mz_m2_charge2_int)    
                
                                    if len(mz_m1_charge2_int)==0:
                                        max_m1_charge2_int = 0.00001
                                    else:
                                        max_m1_charge2_int = max(mz_m1_charge2_int) 
                                            
                                    if len(mz_0_charge2_int)==0:
                                        max_0_charge2_int = 0.00001
                                    else:
                                        max_0_charge2_int = max(mz_0_charge2_int)                                                           
                                        
                                    if len(mz_p1_charge2_int)==0:
                                        max_p1_charge2_int = 0.00001
                                    else:
                                        max_p1_charge2_int = max(mz_p1_charge2_int)   
                                        
                                    if len(mz_p2_charge2_int)==0:
                                        max_p2_charge2_int = 0.00001
                                    else:
                                        max_p2_charge2_int = max(mz_p2_charge2_int)                                                                                       
                                                                                                                                                        
                                    if len(mz_p3_charge2_int)==0:
                                        max_p3_charge2_int = 0.00001
                                    else:
                                        max_p3_charge2_int = max(mz_p3_charge2_int)                                                                                                                                                                                                       
                                                                                                                                                                                                                                                                                                                                
                                    ###############################################
                                    if len(mz_m2_charge3_int)==0:
                                        max_m2_charge3_int = 0.00001
                                    else:
                                        max_m2_charge3_int = max(mz_m2_charge3_int)    
                
                                    if len(mz_m1_charge3_int)==0:
                                        max_m1_charge3_int = 0.00001
                                    else:
                                        max_m1_charge3_int = max(mz_m1_charge3_int) 
                                            
                                    if len(mz_0_charge3_int)==0:
                                        max_0_charge3_int = 0.00001
                                    else:
                                        max_0_charge3_int = max(mz_0_charge3_int)                                                           
                                        
                                    if len(mz_p1_charge3_int)==0:
                                        max_p1_charge3_int = 0.00001
                                    else:
                                        max_p1_charge3_int = max(mz_p1_charge3_int)   
                                        
                                    if len(mz_p2_charge3_int)==0:
                                        max_p2_charge3_int = 0.00001
                                    else:
                                        max_p2_charge3_int = max(mz_p2_charge3_int)                                                                                       
                                                                                                                                                        
                                    if len(mz_p3_charge3_int)==0:
                                        max_p3_charge3_int = 0.00001
                                    else:
                                        max_p3_charge3_int = max(mz_p3_charge3_int)                                                             
                                    ###################################################                                                                                        
                                    if len(mz_m2_charge4_int)==0:
                                        max_m2_charge4_int = 0.00001
                                    else:
                                        max_m2_charge4_int = max(mz_m2_charge4_int)    
                
                                    if len(mz_m1_charge4_int)==0:
                                        max_m1_charge4_int = 0.00001
                                    else:
                                        max_m1_charge4_int = max(mz_m1_charge4_int) 
                                            
                                    if len(mz_0_charge4_int)==0:
                                        max_0_charge4_int = 0.00001
                                    else:
                                        max_0_charge4_int = max(mz_0_charge4_int)                                                           
                                        
                                    if len(mz_p1_charge4_int)==0:
                                        max_p1_charge4_int = 0.00001
                                    else:
                                        max_p1_charge4_int = max(mz_p1_charge4_int)   
                                        
                                    if len(mz_p2_charge4_int)==0:
                                        max_p2_charge4_int = 0.00001
                                    else:
                                        max_p2_charge4_int = max(mz_p2_charge4_int)                                                                                       
                                                                                                                                                        
                                    if len(mz_p3_charge4_int)==0:
                                        max_p3_charge4_int = 0.00001
                                    else:
                                        max_p3_charge4_int = max(mz_p3_charge4_int)                                                                                                                                                                             
                                    ...
                                    if len(mz_m2_charge2_ls) == 0:
                                        mz_m2_charge2_ls = [1]
                                        
                                    if len(mz_m1_charge2_ls) == 0:
                                        mz_m1_charge2_ls = [1]                            
                                        
                                    if len(mz_0_charge2_ls) == 0:
                                        mz_0_charge2_ls = [1]                            
                                        
                                    if len(mz_m2_charge3_ls) == 0:
                                        mz_m2_charge3_ls = [1]                            
                                    
                                    if len(mz_m1_charge3_ls) == 0:
                                        mz_m1_charge3_ls = [1]                            
                                        
                                    if len(mz_0_charge3_ls) == 0:
                                        mz_0_charge3_ls = [1]
                                        
                                    if len(mz_m2_charge4_ls) == 0:
                                        mz_m2_charge4_ls = [1]                                                        
                                                                                                
                                    if len(mz_m1_charge4_ls) == 0:
                                        mz_m1_charge4_ls = [1]                                                                                                                
                                                                                                                                                        
                                    if len(mz_0_charge4_ls) == 0:
                                        mz_0_charge4_ls = [1]                                                                                                                                                                        
                            ...
                            if len(peak_list_in_win) > 0 and len(peak_int_in_win) > 0 and best_match_mz > 300 and initial_maxpeakint > initial_max_peak_intensity_threshold: # control this parameter
                                ...
                                #print removal_list
                                #removal_list = [best_match_mz,best_match_mz+(isotope_dis*1)/best_match_charge]#,best_match_mz+(isotope_dis*2)/best_match_charge]
                                #removal_list = [best_match_mz]
                                #print removal_list
                                #print peak_list_in_win
                                t1=[];t2=[]
                                for x2,y2,x3,y3 in zip(peak_list_in_win,peak_int_in_win,peak_list_in_win_for_charge,peak_int_in_win_for_charge):
                                    removal_switch = 'OFF'
                                ...
                                peak_int_in_win = new_int_list_for_charge          
        ...
        for zz in inputlist1:
            ...
            if ss_count != 3 and write_switch1 == 'ON':    
                if '<indexOffset>' in zz:
                    outputfile.write(' <indexOffset>%d</indexOffset>' %(index_pos))
