...
def isolation_window_determination(ms1ms2list):
    ...
    return [left_isolation_window,right_isolation_window]





def ms2iteration(scan_unit,peak_list,peak_int,left_isolation_window,right_isolation_window,scan_num2):
    ...
    return [temp_list1,scan_num2]


def ms1ms2list_process(ms1ms2list,left_isolation_window,right_isolation_window,scan_num1):
    ...
    for nn, x in enumerate(ms1ms2list):
        ...
        if msLevel == 2 and '</scan>' in x:
            #print scan_unit
            ms2iteration_out = ms2iteration(scan_unit,peak_list,peak_int,left_isolation_window,right_isolation_window,scan_num1)
            corrected_scan_unit = ms2iteration_out[0]
            temp_list.append(corrected_scan_unit)
            scan_num1 = ms2iteration_out[1]
            scan_unit = []
        
    return [temp_list,scan_num1]



def index_mzxml(inputlist2,scan_num10):
    ss_count = 0
    char_count = 0
    scan_num_count = 0
    offset = {}
    write_switch1 = 'ON'
    for zz in inputlist2:
        if '<msRun scanCount=' in zz:
            zz_ls = zz.split('startTime')
            zz = ' <msRun scanCount="' + str(scan_num10) + '" startTime' + zz_ls[1]
        if '</scan>' in zz:
            ss_count += 1
        else:
            ss_count = 0
...
def main():
    ...
    for num, xx in enumerate(inputfile1): # Input file
        ...
        if '</scan>' in previous1 and '</scan>' in xx:
            ...
            left_right_isolation_window = isolation_window_determination(ms1ms2list)
            left_isolation_window = left_right_isolation_window[0]
            right_isolation_window = left_right_isolation_window[1]
            
            corrected_ms1ms2_list_scan_num = ms1ms2list_process(ms1ms2list,left_isolation_window,right_isolation_window,scan_num)    
            corrected_ms1ms2_list = corrected_ms1ms2_list_scan_num[0]
            scan_num = corrected_ms1ms2_list_scan_num[1]
            
            for mm in corrected_ms1ms2_list: inputlist1.append(mm)
            ms1ms2list = []
        previous1 = xx
            
    index_mzxml(inputlist1,scan_num)
